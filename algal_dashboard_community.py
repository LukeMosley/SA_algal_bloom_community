...
def main():
    ...
    folium.Marker(
        location=[0, 0],  # Dummy marker, won't be visible
    ).add_to(m)  # This is just to trigger Font Awesome inclusion
    ...
    cameras = [
        {
            'name': 'West Beach Boat Ramp',
            'lat': -34.957020,
            'lon': 138.508924,
            'url': 'https://www.marinesafety.sa.gov.au/web-cameras/west-beach-web-camera'
        },
        ...
        #     'lat': -35.0,
        #     'lon': 138.5,
        #     'url': 'https://example.com/other-cam'
        # },
    ]
    # Add camera markers
    for cam in cameras:
        popup_html = f"""
            <a href="{cam['url']}" target="_blank" style="text-decoration: none; color: blue;">
                View {cam['name']} Camera
            </a>
        """
        folium.Marker(
            location=[cam['lat'], cam['lon']],
            icon=folium.Icon(icon='camera', prefix='fa', color='blue'),  # 'fa' for Font Awesome; customize color as needed
