...
def main():
    ...
    for _, row in pd.concat([sub_df, comm_sub_df], ignore_index=True).iterrows():
    ...
    # ✅ Use constant key so map layers don't reset
    ...
    if not df.empty:
        ...
        selected_site = st.selectbox("Filter by site", options=["All Sites"] + all_sites, index=0)
        ...
        if not plot_df.empty:
            ...
            trend_melted = trend_df.melt(id_vars='Date_Sample_Collected', var_name='Species', value_name='Cell_Count', ignore_index=False)
    ...
    st.markdown("---")
    st.markdown("© 2025 South Australia HAB Dashboard Prototype. Built with Streamlit and Folium.")
